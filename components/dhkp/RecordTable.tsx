  ...
  if (records.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: 'var(--c-navy-light)', color: 'var(--c-navy)' }}>
          <FileX size={28} />
        </div>
        <div className="text-center">
          <p className="font-semibold" style={{ color: 'var(--c-text-1)', fontSize: 'var(--text-base)' }}>Tidak ada hasil</p>
          <p className="mt-1" style={{ color: 'var(--c-text-3)', fontSize: 'var(--text-sm)' }}>Coba ubah filter atau kata kunci pencarian.</p>
        </div>
      </div>
    );
  }
      ...
      <table className="dhkp-table">
          ...
          {records.map((record, idx) => {
            ...
            return (
              <tr key={record.id} className={record.statusLunas ? 'row-lunas' : ''}>
                ...
                <td className="col-sticky-left text-center font-medium" style={{ color: 'var(--c-text-3)', fontSize: 'var(--text-sm)' }}>
                  {startIdx + idx + 1}
                </td>

                <td style={{ fontSize: 'var(--text-sm)', fontFamily: 'monospace' }}>
                  <div className="flex items-center gap-1.5">
                    ...
                    {record.nop && (
                      <button type="button" onClick={() => toggleReveal(record.id)}
                        ...
                        style={{ width: 24, height: 24, color: 'var(--c-text-4)' }}>
                ...
                <td>
                  <span className="font-semibold" style={{ color: 'var(--c-text-1)', fontSize: 'var(--text-sm)', whiteSpace: 'nowrap' }}>
                    {record.namaWajibPajak}
                  </span>
                </td>

                <td>
                  <span className="block truncate" style={{ color: 'var(--c-text-3)', fontSize: 'var(--text-sm)', maxWidth: 180 }} title={record.alamatObjekPajak}>
                    {record.alamatObjekPajak || '-'}
                  </span>
                </td>

                <td className="col-number">
                  <span className="font-semibold" style={{ color: 'var(--c-navy)', fontSize: 'var(--text-sm)', whiteSpace: 'nowrap' }}>
                ...
                <td className="col-number">
                  <span style={{
                    ...
                    color: record.perubahanPajak > 0 ? 'var(--c-success)' : record.perubahanPajak < 0 ? 'var(--c-danger)' : 'var(--c-text-3)',
                  }}>
                ...
                <td style={{ color: 'var(--c-text-3)', fontSize: 'var(--text-sm)', whiteSpace: 'nowrap' }}>
                  {formatTanggal(record.tanggalBayar)}
                </td>

                <td className="col-number" style={{ fontSize: 'var(--text-sm)', whiteSpace: 'nowrap' }}>
                ...
                <td className="col-sticky-right">
                  <div className="flex items-center justify-center gap-2">
                    <button onClick={() => onEdit(record)} disabled={isLocked} title="Edit data" aria-label="Edit data"
                      ...
                      style={{ width: 40, height: 40, background: 'var(--c-navy-light)', color: 'var(--c-navy)' }}>
                      <Pencil size={16} />
                    </button>
                    <button onClick={() => onDelete(record)} disabled={isLocked} title="Hapus data" aria-label="Hapus data"
                      className="flex items-center justify-center rounded-md transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                      style={{ width: 40, height: 40, background: 'var(--c-danger-light)', color: 'var(--c-danger)' }}>
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
        ...
        <tfoot>
          <tr style={{ background: 'var(--c-surface-2)' }}>
            <td colSpan={5} className="text-right py-3 px-3 font-semibold" style={{ color: 'var(--c-text-3)', fontSize: 'var(--text-sm)' }}>
              Total Halaman Ini
            </td>
            <td className="col-number py-3 px-3 font-bold" style={{ color: 'var(--c-navy)', fontSize: 'var(--text-sm)', whiteSpace: 'nowrap' }}>
              {formatRupiah(totalPajakPage)}
            </td>
            <td colSpan={7} />
          </tr>
          <tr style={{ background: 'var(--c-navy-light)' }}>
            <td colSpan={5} className="text-right py-3 px-3 font-bold" style={{ color: 'var(--c-navy)', fontSize: 'var(--text-sm)' }}>
